...
| `Old_CS.ipynb` | Lorenz | Bidirectional, all 3 states | Complete synchronization (CS): mean-absolute-error metric, sweeps coupling strength α, small trial count. Full-state coupling synchronizes above αc ≈ 0.453. |
...
| `x_coupling_CS.ipynb` | Lorenz | Bidirectional, **x-only** | CS with coupling injected through the x-variable alone (y, z uncoupled) — raises the synchronization threshold to α ≈ 3.80. |
| `GraphAnnotation.ipynb` | Lorenz | — | Reads a pre-computed error-vs-α CSV (`For_Fujisaka_E_new.csv`) and annotates α*c ≈ 11.8 — the *second* threshold beyond which non-uniform-chaos deviations (some initial conditions failing to sync even above the primary αc ≈ 0.453) disappear entirely. |
| `Phase Sync-1.ipynb` | Rössler (detuned, ω₁≠ω₂) | Bidirectional | Average frequency difference ΔΩ vs coupling strength α — locates the phase-locking onset at α ≈ 0.04. |
